...
import { LogbookModule } from '@/components/student/LogbookModule';
import { AttendanceIAModule } from '@/components/student/AttendanceIAModule';
import { ClinicalPostingsModule } from '@/components/student/ClinicalPostingsModule';
import { FeesHostelModule } from '@/components/student/FeesHostelModule';

export interface StudentDashboardViewProps {
  initialTab?: 'overview' | 'logbook' | 'attendance' | 'postings' | 'fees';
}
...
export const StudentDashboardView: React.FC<StudentDashboardViewProps> = ({ initialTab = 'overview' }) => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'logbook' | 'attendance' | 'postings' | 'fees'>(initialTab);

  // Quick log props state passing
  const [prefilledWard, setPrefilledWard] = useState<string>('');
  const [prefilledFaculty, setPrefilledFaculty] = useState<string>('');
  ...
  const handleQuickLogFromPostings = (ward: string, faculty: string) => {
    setPrefilledWard(ward);
    setPrefilledFaculty(faculty);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="erp-card p-2.5 bg-white border-b border-slate-200 shadow-sm flex items-center justify-between overflow-x-auto">
        <div className="flex gap-1 min-w-max">
          ...
          >
            ...
            <span>2. My Attendance & IA 40%</span>
          </button>
          <button
          ...
          >
            ...
            <span>3. My Clinical Postings</span>
          </button>
          <button
          ...
          >
            ...
            <span>4. My Fees & Hostel</span>
          </button>
        </div>

        <div className="hidden xl:flex items-center gap-2 text-[11px] font-bold text-slate-500 px-3">
          <ShieldCheck className="w-4 h-4 text-emerald-600" />
          <span>NMC CBME Compliant Portal</span>
        </div>
      </div>
      ...
      {activeTab === 'logbook' && (
        <LogbookModule
          initialWard={prefilledWard}
          initialFaculty={prefilledFaculty}
          onNavigateToPostings={() => setActiveTab('postings')}
        />
      )}

      {activeTab === 'attendance' && <AttendanceIAModule />}

      {activeTab === 'postings' && (
        <ClinicalPostingsModule onQuickLog={handleQuickLogFromPostings} />
      )}

      {activeTab === 'fees' && <FeesHostelModule />}

      {/* OVERVIEW TAB (Default Dashboard View) */}
      {activeTab === 'overview' && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            >
              ...
              <div className="text-[11px] text-slate-500 font-medium">Pending Balance: ₹15,000</div>
            </div>
